...
TOKEN_TELEGRAM = '5854763694:AAGkThC63biSdmb1xcabg9_fbDUBTvAspdU'  #Telegram token id
CHAT_ID_TELEGRAM = '-1001740687772'  #Telegram Chat ID
Local_NODE = 'IN 1'  #Just a reference number when running multiple instances of the same script at the same time...just an identifier
webhook_url = 'X'
DISCORD = False
TELEGRAM = True
...
def browserSetup(isMobile: bool,
                 user_agent: str = PC_USER_AGENT,
                 proxy: str = None) -> WebDriver:
...
def login(browser: WebDriver, email: str, pwd: str, isMobile: bool = False):
  ...
  if ARGS.session:
    if browser.title == "We're updating our terms" or isElementExists(
        browser, By.ID, 'iAccrualForm'):
      time.sleep(2)
      browser.find_element(By.ID, 'iNext').click()
      time.sleep(5)
    if browser.title == 'Is your security info still accurate?' or isElementExists(
        browser, By.ID, 'iLooksGood'):
      time.sleep(2)
    ...
    if isElementExists(browser, By.ID, "setupAppDesc"):
      ...
      time.sleep(5)
    if browser.title == 'Microsoft account | Home' or isElementExists(
        browser, By.ID, 'navs_container'):
      ...
      if isElementExists("iShowSkip") and ARGS.skip_unusual:
        ...
        updateLogs()
        cleanLogs()
        if TELEGRAM == True or DISCORD == True:
          message = createMessage()
          sendReportToMessenger(message)
        input('Press any key to close...')
        os._exit(0)
    elif isElementExists(
        browser, By.ID,
        'mectrl_headerPicture') or 'Sign In or Create' in browser.title:
  ...
  try:
    if browser.title == "":
      ...
      wait.until(
        ec.presence_of_element_located((By.CSS_SELECTOR, "html[lang]")))
    ...
    if browser.title == "Your account has been temporarily suspended" or isElementExists(
        browser, By.CLASS_NAME, "serviceAbusePageContainer  PageContainer"):
      LOGS[CURRENT_ACCOUNT]['Last check'] = 'Your account has been locked !'
      FINISHED_ACCOUNTS.append(CURRENT_ACCOUNT)
      updateLogs()
      cleanLogs()
...
def RewardsLogin(browser: WebDriver):
  ...
  try:
    time.sleep(
      10 if not FAST and not SUPER_FAST else 5 if not SUPER_FAST else 2.5)
...
def checkBingLogin(browser: WebDriver, isMobile: bool = False):
  ...
  # Wait 15 seconds
  time.sleep(
    15 if not FAST and not SUPER_FAST else 10 if not SUPER_FAST else 5)
      ...
      if not isMobile:
        try:
          ...
          if browser.find_element(By.ID, 'id_s').is_displayed():
            ...
            time.sleep(15 if not FAST and not SUPER_FAST else
                       7 if not SUPER_FAST else 3)
            checkBingLogin(browser, isMobile)
          time.sleep(2)
          POINTS_COUNTER = int(
            browser.find_element(By.ID,
                                 "id_rc").get_attribute("innerHTML").replace(
                                   ",", ""))
      else:
  ...
  if isMobile:
    ...
    if isElementExists(browser, By.ID, 'bnp_rich_div'):
      try:
        browser.find_element(By.XPATH,
                             '//*[@id="bnp_bop_close_icon"]/img').click()
      except NoSuchElementException:
        pass
    try:
      time.sleep(1)
      ...
      try:
        ...
        pass
      time.sleep(1)
      if isElementExists(browser, By.XPATH,
    ...
    try:
      ...
      if str(browser.current_url).split(
          '?')[0] == "https://account.live.com/proofs/Add":
        prRed(
          f'[LOGIN] Please complete the Security Check on {CURRENT_ACCOUNT}')
        FINISHED_ACCOUNTS.append(CURRENT_ACCOUNT)
...
def waitUntilVisible(browser: WebDriver,
                     by_: By,
                     selector: str,
                     time_to_wait: int = 10):
  """Wait until visible"""
...
def waitUntilClickable(browser: WebDriver,
                       by_: By,
                       selector: str,
                       time_to_wait: int = 10):
  """Wait until clickable"""
  WebDriverWait(browser,
                time_to_wait).until(ec.element_to_be_clickable(
                  (by_, selector)))
...
def getGoogleTrends(numberOfwords: int) -> list:
  ...
  while len(search_terms) < numberOfwords:
    i += 1
    r = requests.get('https://trends.google.com/trends/api/dailytrends?hl=' +
                     LANG + '&ed=' +
                     str((date.today() -
                          timedelta(days=i)).strftime('%Y%m%d')) + '&geo=' +
                     GEO + '&ns=15')
...
def bingSearches(browser: WebDriver,
  ...
  try:
    ...
    if len(search_terms) == 0:
      try:
        words = open(f"{Path.cwd().resolve()}/searchwords.txt",
                     "r").read().splitlines()
...
def bingSearch(browser: WebDriver, word: str, isMobile: bool):
  ...
  searchbar.submit()
  time.sleep(
    random.randint(12, 24) if not FAST and not SUPER_FAST else random.
    randint(6, 9) if not SUPER_FAST else 3)
  points = 0
  try:
    if not isMobile:
      try:
...
def completePromotionalItems(browser: WebDriver):
  ...
  try:
    ...
    if (item["pointProgressMax"] == 100 or item["pointProgressMax"] == 200
        ) and item["complete"] is False and item["destinationUrl"] == BASE_URL:
      browser.find_element(
        By.XPATH, '//*[@id="promo-item"]/section/div/div/div/a').click()
      time.sleep(1)
      browser.switch_to.window(window_name=browser.window_handles[1])
      time.sleep(
        8 if not FAST and not SUPER_FAST else 5 if not SUPER_FAST else 2.5)
      browser.close()
      time.sleep(2)
...
def completeDailySetSearch(browser: WebDriver, cardNumber: int):
  ...
  browser.find_element(
    By.XPATH,
    f'//*[@id="app-host"]/ui-view/mee-rewards-dashboard/main/div/mee-rewards-daily-set-section/div/mee-card-group/div/mee-card[{str(cardNumber)}]/div/card-content/mee-rewards-daily-set-item-content/div/a/div/span'
  ).click()
  time.sleep(1)
  browser.switch_to.window(window_name=browser.window_handles[1])
  time.sleep(
...
def completeDailySetQuiz(browser: WebDriver, cardNumber: int):
  ...
  browser.find_element(
    By.XPATH,
    f'//*[@id="app-host"]/ui-view/mee-rewards-dashboard/main/div/mee-rewards-daily-set-section[1]/div/mee-card-group[1]/div[1]/mee-card[{str(cardNumber)}]/div[1]/card-content[1]/mee-rewards-daily-set-item-content[1]/div[1]/a[1]/div[3]/span[1]'
  ).click()
  time.sleep(3)
  browser.switch_to.window(window_name=browser.window_handles[1])
  time.sleep(12 if not FAST and not SUPER_FAST else random.
             randint(5, 8) if not SUPER_FAST else 3)
  ...
  waitUntilVisible(browser, By.XPATH,
                   ...
                   10 if not FAST and not SUPER_FAST else 5)
  time.sleep(3)
    ...
    if numberOfOptions == 8:
      ...
      for i in range(8):
        if browser.find_element(
            ...
            str(i)).get_attribute("iscorrectoption").lower() == "true":
...
def completeDailySetVariableActivity(browser: WebDriver, cardNumber: int):
  ...
  correctAnswer = browser.execute_script(
    "return _w.rewardsQuizRenderInfo.correctAnswer")
  if browser.find_element(
      By.ID, "rqAnswerOption0").get_attribute("data-option") == correctAnswer:
...
def completeDailySetThisOrThat(browser: WebDriver, cardNumber: int):
  ...
  browser.find_element(By.XPATH, '//*[@id="rqStartQuiz"]').click()
  waitUntilVisible(
    browser, By.XPATH, '//*[@id="currentQuestionContainer"]/div/div[1]',
    15 if not FAST and not SUPER_FAST else 10 if not SUPER_FAST else 5)
  time.sleep(5)
  for _ in range(10):
    # Click on later on Bing wallpaper app popup
    ...
    try:
      if not activity['complete']:
        ...
        if activity['promotionType'] == "quiz":
          if activity['pointProgressMax'] == 50 and activity[
              'pointProgress'] == 0:
            print('[DAILY SET]',
            ...
            completeDailySetThisOrThat(browser, cardNumber)
                ...
                or activity['pointProgressMax']
                == 30) and activity['pointProgress'] == 0:
            print('[DAILY SET]', 'Completing quiz of card ' + str(cardNumber))
            completeDailySetQuiz(browser, cardNumber)
          elif activity['pointProgressMax'] == 10 and activity[
              'pointProgress'] == 0:
            searchUrl = urllib.parse.unquote(
              urllib.parse.parse_qs(
                urllib.parse.urlparse(
                  activity['destinationUrl']).query)['ru'][0])
  ...
  for child in childPromotions:
    if not child['complete']:
      ...
      if child['promotionType'] == "quiz" and child['pointProgressMax'] >= 50:
        ...
        time.sleep(5)
        waitUntilVisible(
          browser, By.XPATH, '//*[@id="currentQuestionContainer"]',
          15 if not FAST and not SUPER_FAST else 10 if not SUPER_FAST else 5)
        numberOfQuestions = browser.execute_script(
          "return _w.rewardsQuizRenderInfo.maxQuestions")
        ...
        counter = str(
          browser.find_element(
            By.XPATH, '//*[@id="QuestionPane0"]/div[2]').get_attribute(
              'innerHTML'))[:-1][1:]
...
def completePunchCards(browser: WebDriver):
  ...
  for punchCard in punchCards:
    try:
      if punchCard['parentPromotion'] != None and punchCard[
          'childPromotions'] != None and punchCard['parentPromotion'][
            'complete'] is False and punchCard['parentPromotion'][
              'pointProgressMax'] != 0:
...
def completeMorePromotionSearch(browser: WebDriver, cardNumber: int):
  ...
  browser.find_element(
    By.XPATH,
    f'//*[@id="app-host"]/ui-view/mee-rewards-dashboard/main/div/mee-rewards-more-activities-card/mee-card-group/div/mee-card[{str(cardNumber)}]/div/card-content/mee-rewards-more-activities-card-item/div/a/div/span'
  ).click()
  time.sleep(1)
  browser.switch_to.window(window_name=browser.window_handles[1])
  time.sleep(
...
def completeMorePromotionQuiz(browser: WebDriver, cardNumber: int):
  ...
  browser.switch_to.window(window_name=browser.window_handles[1])
  time.sleep(
    10 if not FAST and not SUPER_FAST else 7 if not SUPER_FAST else 3.5)
  if not waitUntilQuizLoads(browser):
    resetTabs(browser)
    return
  CurrentQuestionNumber = browser.execute_script(
    "return _w.rewardsQuizRenderInfo.currentQuestionNumber")
  if CurrentQuestionNumber == 1 and isElementExists(browser, By.XPATH,
                                                    '//*[@id="rqStartQuiz"]'):
    browser.find_element(By.XPATH, '//*[@id="rqStartQuiz"]').click()
  waitUntilVisible(
    browser, By.XPATH, '//*[@id="currentQuestionContainer"]/div/div[1]',
...
def completeMorePromotionThisOrThat(browser: WebDriver, cardNumber: int):
  ...
  if CrrentQuestionNumber == 1 and isElementExists(browser, By.XPATH,
                                                   '//*[@id="rqStartQuiz"]'):
    browser.find_element(By.XPATH, '//*[@id="rqStartQuiz"]').click()
  waitUntilVisible(
    browser, By.XPATH, '//*[@id="currentQuestionContainer"]/div/div[1]',
    10 if not FAST and not SUPER_FAST else 7 if not SUPER_FAST else 5)
  time.sleep(3)
    ...
    try:
      ...
      if promotion['complete'] is False and promotion['pointProgressMax'] != 0:
        if promotion['promotionType'] == "urlreward":
          ...
          elif promotion['pointProgressMax'] == 30 or promotion[
              'pointProgressMax'] == 40:
            completeMorePromotionQuiz(browser, i)
          elif promotion['pointProgressMax'] == 50:
            completeMorePromotionThisOrThat(browser, i)
        else:
          if promotion['pointProgressMax'] == 100 or promotion[
...
def completeMSNShoppingGame(browser: WebDriver):
  ...
  def expandShadowElement(element,
    ...
    if index is not None:
      shadow_root = WebDriverWait(browser, 45).until(
        ec.visibility_of(
          browser.execute_script('return arguments[0].shadowRoot.children',
                                 element)[index]))
    else:
  ...
  def signIn() -> None:
    ...
    waitUntilVisible(
      browser, By.TAG_NAME, 'shopping-page-base',
      60 if not FAST and not SUPER_FAST else 30 if not SUPER_FAST else 5)
    expandShadowElement(
  ...
  def getGamingCard() -> Union[WebElement, Literal[False]]:
    ...
    msft_feed_layout = expandShadowElement(shopping_homepage, 0).find_element(
      By.TAG_NAME, 'msft-feed-layout')
  ...
  def clickPlayAgain() -> None:
    ...
    options_container = expandShadowElement(gaming_card)[1]
    getChildren(options_container)[0].find_element(By.TAG_NAME,
                                                   'button').click()
  ...
  try:
    ...
    while tries <= 4:
      ...
      waitUntilVisible(
        browser, By.TAG_NAME, 'shopping-page-base',
        45 if not FAST and not SUPER_FAST else 30 if not SUPER_FAST else 5)
      time.sleep(
        15 if not FAST and not SUPER_FAST else 10 if not SUPER_FAST else 5)
      try:
        sign_in_button = getSignInButton()
    ...
    while not gaming_card and scrolls <= 5:
      ...
      time.sleep(
        10 if not FAST and not SUPER_FAST else 5 if not SUPER_FAST else 2)
      gaming_card = getGamingCard()
      if gaming_card:
...
def getRemainingSearches(browser: WebDriver):
  ...
  if not 'pcSearch' in counters:
    return 0, 0
  progressDesktop = counters['pcSearch'][0]['pointProgress'] + counters[
    'pcSearch'][1]['pointProgress']
  targetDesktop = counters['pcSearch'][0]['pointProgressMax'] + counters[
    'pcSearch'][1]['pointProgressMax']
  if targetDesktop == 33:
    # Level 1 EU
    searchPoints = 3
...
def argumentParser():
  ...
  parser = ArgumentParser(
    description="Microsoft Rewards Farmer V3.1",
    allow_abbrev=False,
    usage=
    "You may use execute the program with the default config or use arguments to configure available options."
  )
  parser.add_argument(
    '--everyday',
    action='store_true',
    help=
    'This argument will make the script run everyday at the time you start.',
    required=False)
  ...
  parser.add_argument(
    '--superfast',
    help=
    "Reduce delays SUPER FAST where ever it's possible to make script faster.",
    action='store_true',
    required=False)
  ...
  parser.add_argument(
    "--on-finish",
    help=
    "Action to perform on finish from one of the following: shutdown, sleep, hibernate, exit",
  ...
  parser.add_argument(
    "--skip-shopping",
    help=
    "Skip MSN shopping game (useful for people living in regions which do not support MSN Shopping.",
    action="store_true",
    required=False)
...
def logs():
  ...
  try:
    ...
    LOGS = json.load(
      open(f"{Path(__file__).parent}/Logs_{account_path.stem}.txt", "r"))
    ...
    for account in LOGS.keys():
      if LOGS[account]["Last check"] == str(date.today()) and list(
          LOGS[account].keys()) == ['Last check', "Today's points", 'Points']:
        FINISHED_ACCOUNTS.append(account)
      elif LOGS[account]['Last check'] == 'Your account has been suspended':
        FINISHED_ACCOUNTS.append(account)
      elif LOGS[account]['Last check'] == str(date.today()) and list(
          LOGS[account].keys()) == [
            'Last check', "Today's points", 'Points', 'Daily', 'Punch cards',
            'More promotions', 'MSN shopping game', 'PC searches'
          ]:
        continue
      else:
...
def updateLogs():
  ...
  with open(f'{Path(__file__).parent}/Logs_{account_path.stem}.txt',
            'w') as file:
...
def finishedAccount():
  ...
  FINISHED_ACCOUNTS.append(CURRENT_ACCOUNT)
  if LOGS[CURRENT_ACCOUNT]["Points"] > 0 and POINTS_COUNTER >= LOGS[
      CURRENT_ACCOUNT]["Points"]:
    LOGS[CURRENT_ACCOUNT][
      "Today's points"] = POINTS_COUNTER - LOGS[CURRENT_ACCOUNT]["Points"]
  else:
...
def createMessage():
  ...
  total_earned = 0
  total_overall = 0
  message = f'📅 Daily report {today} {Local_NODE}\n\n'
...
def sendReportToMessenger(message):
  ...
  if TELEGRAM == True:
    sendReportToTelegram(message)
  if DISCORD == True:
    sendToDiscord(message)


def sendReportToTelegram(message):
  if TELEGRAM == True:
    t = get_notifier('telegram')
    t.notify(message=message, token=TOKEN_TELEGRAM, chat_id=CHAT_ID_TELEGRAM)
...
def setRedeemGoal(browser: WebDriver, goal: str):
  ...
  try:
    goal_name = browser.find_element(
      By.XPATH,
      value=
      "/html/body/div[1]/div[2]/main/div/ui-view/mee-rewards-dashboard/main/div/mee-rewards-redeem-info-card/div/mee-card-group/div/div[1]/mee-card/div/card-content/mee-rewards-redeem-goal-card/div/div[2]/h3",
    )

    goal_progress = browser.find_element(
      By.XPATH,
      value=
      "/html/body/div[1]/div[2]/main/div/ui-view/mee-rewards-dashboard/main/div/mee-rewards-redeem-info-card/div/mee-card-group/div/div[1]/mee-card/div/card-content/mee-rewards-redeem-goal-card/div/div[2]/p",
    )

    # If goal is not set or is not the specified one, then set/change it
    if "/" not in goal_progress.text.lower(
    ) or goal not in goal_name.text.lower():
      # If we need to change it, it is mandatory to refresh the set goal button
      if "/" in goal_progress.text.lower(
      ) and goal not in goal_name.text.lower():
        ...
        if points == total:
          ...
          element = browser.find_element(
            By.XPATH,
            value=
            "/html/body/div[1]/div[2]/main/div/ui-view/mee-rewards-dashboard/main/div/mee-rewards-redeem-info-card/div/mee-card-group/div/div[1]/mee-card/div/card-content/mee-rewards-redeem-goal-card/div/div[2]/div/a[2]/span/ng-transclude",
          )
        else:
          element = browser.find_element(
            By.XPATH,
            value=
            "/html/body/div[1]/div[2]/main/div/ui-view/mee-rewards-dashboard/main/div/mee-rewards-redeem-info-card/div/mee-card-group/div/div[1]/mee-card/div/card-content/mee-rewards-redeem-goal-card/div/div[2]/div/a/span/ng-transclude",
          )
  ...
  try:
    try:
      browser.find_element(
        By.XPATH,
        value=
        "/html/body/div[1]/div[2]/main/div/ui-view/mee-rewards-dashboard/main/div/mee-rewards-redeem-info-card/div/mee-card-group/div/div[1]/mee-card/div/card-content/mee-rewards-redeem-goal-card/div/div[2]/div/a[1]",
      ).click()
    ...
    try:
      ...
      try:
        browser.find_element(By.XPATH,
                             value=f'//*[@id="redeem-pdp_{id}"]').click()
        time.sleep(random.uniform(5, 7))
      except NoSuchElementException:
        browser.find_element(
          By.XPATH, value=f'//*[@id="redeem-pdp_{id}"]/span[1]').click()
      ...
      try:
        ...
        browser.find_element(
          By.XPATH,
          value='//*[@id="redeem-checkout-review-confirm"]/span[1]').click()
...
def logo():
  ...
  prPurple(
    "        by @Farshadz1997 upgraded by @Ghosty-Gigabytes        version 3.1\n"
  )
  prYellow('*************************[NODE ' + Local_NODE +
           ']*************************')

  if os.path.exists("Logs_accounts.txt"):
    os.remove("Logs_accounts.txt")
...
def tkinter_calculator():
  ...
  price_entry.configure(
    validatecommand=(price_entry.register(validate_float_input), '%P'))
  ...
  accounts_entry.configure(
    validatecommand=(accounts_entry.register(validate_numeric_input), '%P'))
  ...
  balance_entry.configure(
    validatecommand=(balance_entry.register(validate_float_input), '%P'))
  ...
  estimated_daily_points.configure(
    validatecommand=(estimated_daily_points.register(validate_float_input),
                     '%P'))

  # Add a label for the associated field
  associated_label = ttk.Label(form_frame,
                               text="Microsoft Associated Gift Card:")
  ...
  yes_radio = ttk.Radiobutton(form_frame,
                              text="Yes",
                              variable=associated_var,
                              value=True)
  no_radio = ttk.Radiobutton(form_frame,
                             text="No",
                             variable=associated_var,
                             value=False)
  ...
  def submit():
    ...
    elapsed_time = ceil(
      ((microsoft if associated else non_microsoft) / daily_points) *
      cr_per_acc)

    if cards_required <= 0:
      messagebox.showerror("Error",
                           "Current balance is higher or equal to price.")
      return
    ...
    f.write(
      json.dumps([{
        ...
        "password": "Your Password"
                 ...
                 indent=4))
  prPurple(
    f"[ACCOUNT] Accounts credential file '{account_path.name}' created."
  ...
  try:
    for account in ACCOUNTS:
      ...
      prYellow('********************' + CURRENT_ACCOUNT +
               '********************')
      if TELEGRAM == True:
        base_url = 'https://api.telegram.org/bot' + TOKEN_TELEGRAM + '/sendMessage?chat_id=' + CHAT_ID_TELEGRAM + '&text=[NODE ' + Local_NODE + '] Account being farmed ' + CURRENT_ACCOUNT
      ...
      if not LOGS[CURRENT_ACCOUNT]['PC searches']:
        ...
        if ARGS.redeem:
          if 'goal' in account and not account['goal'].lower(
          ) in redeem_goal_title:
        ...
        if ARGS.skip_shopping and not LOGS[CURRENT_ACCOUNT][
            'MSN shopping game']:
      ...
      if MOBILE:
        browser = browserSetup(
          True, account.get('mobile_user_agent', MOBILE_USER_AGENT),
          account.get('proxy', None))
